...
### 4.2 AI 선제 라벨링 가이드 및 인간 분석가 전수 재확인 파이프라인
시스템이 일률적으로 부여한 감성 라벨의 모호성을 타파하기 위해 **[AI 1차 기계 분류 ➡️ 인간 분석가 정밀 전수 검증 및 재확인]**으로 이어지는 고도화된 데이터 레이블링 파이프라인을 구축하였다.

1. **AI 가이드라인 레이블링:** 추출된 2,000건의 샘플에 대해 사전 학습 언어 모델을 활용해 문맥적 뉘앙스를 확률값으로 1차 계산하여 분류 가이드를 생성하였다.
2. **인간 분석가 전수 재확인 및 클래스 확장 (2개 ➡️ 4개):** 분석가가 AI의 1차 분류 결과와 실제 텍스트 내용을 직접 눈으로 전수 대조하며, 이진 감성으로 표현 불가능한 영역을 **'중립(2)' 및 '관계없음(3)' 라벨로 수작업 재분류**하였다. 단순 단어 매칭이 아닌 금융·산업적 맥락과 유저 심리를 반영한 전수 실측 결과 데이터 분포는 다음과 같다.
